...
class GenerationConfig:
    ...
    auto_cutout_label: Optional[str] = None  # default class name for auto-cutout (default: auto)
    auto_cutout_label_from_subdir: bool = False  # use first subdir under objects/ as class
    rectangle_mask_mode: str = "grabcut"  # grabcut | reject | legacy; detector bbox must not become a rectangular alpha tile
    ...
    # Scene-understanding policy and active hard-case generation.
    scene_region_mode: str = "auto"       # auto | explicit | ground-prior | none
    hardcase_recipe: Optional[str] = None  # built-in name or JSON path
...
def validate_config(config: GenerationConfig) -> None:
    ...
    if config.rectangle_mask_mode not in {"grabcut", "reject", "legacy"}:
    ...
    if config.scene_region_mode not in {"auto", "explicit", "ground-prior", "none"}:
        raise ValueError("scene_region_mode 必须是 auto / explicit / ground-prior / none")
    if config.hardcase_recipe:
        from .planning import load_hardcase_recipe
        load_hardcase_recipe(config.hardcase_recipe)

...
from .geometry import annotation_canvas_mask, canvas_polygon, mask_bbox, mask_to_polygons, visible_instance_masks
from .io import imread_with_exif, imwrite_unicode, list_backgrounds, log_default, save_cutouts
from .labelme import load_object_assets, load_paste_zone_masks
...
def _load_assets(config: GenerationConfig, log: Callable[[str], None]) -> List[ObjectAsset]:
    if config.auto_cutout:
        return load_object_assets_auto(config.objects_dir, config.class_map, log)
    return load_object_assets(config.objects_dir, config.class_map, config.feather_sigma, log)
...
def _write_task(plan: SamplePlan) -> TaskResult:
    ...
    try:
        ...
        zone_mask, class_zone_masks = load_paste_zone_masks(bg_path, height, width)
        boxes = []
        annotations = []
        ...
        visible_masks = visible_instance_masks(annotations, width, height) if fmt in {"seg", "both", "coco", "semantic", "obb", "all"} else None
        visibility_rows = []
        if visible_masks is not None:
            raw_masks = [annotation_canvas_mask(a, t, width, height) for a, _b, _f, t in annotations]
            ...
            # Avoid the mask projection cost for Detect-only million-scale runs.
            # Diagnostics report visibility as unavailable rather than pretending it is 1.0.
            visibility_rows = []
        ...
        # Deterministic preview decision per task; no cross-worker RNG state.
        ...
        _atomic_json(ctx.fragment_root / "meta" / f"{plan.index:09d}.json",
...
def _plan_objects(cfg: GenerationConfig, rng: random.Random,
                  profile: Optional[DistributionProfile], template_data: Optional[dict]) -> List[PlannedObject]:
    ...
    if template_data is not None:
        ...
        if not sampled:
            sampled = sample_template(template_data, rng, cfg.class_map)
        return [PlannedObject(PlacementSpec(
            class_id=p.class_id, label=p.label, source_name=p.source_name,
            center_x_ratio=p.cx_ratio, bottom_y_ratio=p.bottom_y_ratio,
            height_ratio=p.h_ratio, flip=p.flip, angle=p.angle,
            allow_overlap=p.allow_overlap, same_class_random=p.same_class_random,
        )) for p in sampled]

    use_profile = profile is not None and rng.random() < cfg.profile_strength
    if use_profile:
        ...
        for _ in range(n):
            ...
            if label is None:
                rows.append(PlannedObject(PlacementSpec()))
                continue
            pos = profile.sample_placement(label, rng)
            rows.append(PlannedObject(PlacementSpec(
                class_id=int(cfg.class_map[label]), label=label,
                center_x_ratio=pos["center_x_ratio"], bottom_y_ratio=pos["bottom_y_ratio"],
                height_ratio=pos["height_ratio"], allow_overlap=bool(pos.get("allow_overlap", False)),
            )))
        return rows
    return [PlannedObject(PlacementSpec()) for _ in range(rng.randint(cfg.min_objects, cfg.max_objects))]
...
def generate_dataset_advanced(config: GenerationConfig, log: Callable[[str], None] = log_default,
    ...
    def plans():
        for idx in range(config.count):
            ...
            objs = _plan_objects(config, rng, profile, template_data)
    ...
    final_live = _write_live_status(output, run_id, status=status, completed=counts["completed"],

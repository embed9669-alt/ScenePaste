    ...
    *final* geometry (used by segmentation/COCO writers to derive visible
    ...
    for attempt in range(PLACEMENT_MAX_ATTEMPTS):
        if placement is not None and placement.center_x_ratio is not None and placement.bottom_y_ratio is not None:
            # Label-first planning proposes an exact normalized location. Explicit
            # scene/class placement masks remain a hard physical constraint: when
            # the proposed foot point is outside the allowed region, retries are
            # resampled from that region rather than silently violating it.
            if attempt == 0:
                xr = float(placement.center_x_ratio)
                yr = float(placement.bottom_y_ratio)
                bottom_x = int(round(np.clip(xr, 0.01, 0.99) * width))
                bottom_y = int(round(np.clip(yr, 0.01, 0.995) * height))
                if zone_mask is not None:
                    zx = int(np.clip(bottom_x, 0, width - 1))
                    zy = int(np.clip(bottom_y, 0, height - 1))
                    if zone_mask[zy, zx] <= 0:

...
class ScenePasteProject:
    ...
    def validate(self, require_generation_paths: bool = False) -> dict:
        ...
        errors = []
        for key, (path_value, expected) in path_specs.items():
            if path_value is None:
                checks[key] = {"configured": False, "exists": None, "kind_ok": None, "path": None}
                continue
            exists = path_value.exists()
            if expected == "dir":
                kind_ok = path_value.is_dir() if exists else False
            elif expected == "file":
                kind_ok = path_value.is_file() if exists else False
            else:
                # Output may not exist yet, but an existing output must be a directory.
                kind_ok = (not exists) or path_value.is_dir()
            checks[key] = {
                "configured": True, "exists": exists, "kind_ok": kind_ok, "path": str(path_value)
            }
        ...
        if "rectangle_mask_mode" in defaults and defaults["rectangle_mask_mode"] not in {"grabcut", "reject", "legacy"}:
            errors.append("defaults.rectangle_mask_mode is invalid")
        if "scene_region_mode" in defaults and defaults["scene_region_mode"] not in {"auto", "explicit", "ground-prior", "none"}:
            errors.append("defaults.scene_region_mode is invalid")
        if defaults.get("hardcase_recipe"):
            try:
                from .core.planning import load_hardcase_recipe
                value = defaults["hardcase_recipe"]
                if isinstance(value, str):
                    p = Path(value).expanduser()
                    if not p.is_absolute() and (self.base_dir / p).is_file():
                        value = str((self.base_dir / p).resolve())
                load_hardcase_recipe(value)
            except Exception as exc:
                errors.append(f"defaults.hardcase_recipe is invalid: {exc}")
        ...
        for key, loader_name in (("augmentation_recipe", "scene"), ("object_appearance_recipe", "object")):
            recipe_value = defaults.get(key)
            if not recipe_value:
                continue
            if isinstance(recipe_value, str):
                p = Path(recipe_value).expanduser()
                source: str | Path | Mapping[str, object] = (
                    self.base_dir / p
                    if (not p.is_absolute() and (self.base_dir / p).is_file())
                    else recipe_value
                )
            elif isinstance(recipe_value, Path):
                source = recipe_value
            elif isinstance(recipe_value, Mapping):
                source = recipe_value
            else:
                errors.append(f"defaults.{key} must be a recipe name, path, or object")
                continue
            try:
...
def init_project(path: Path, *, name: Optional[str] = None, objects: Optional[Path] = None,
    ...
    project = ScenePasteProject(
        ...
        defaults={
            "workers": 0, "output_format": "all", "preview_ratio": 0.01,
            "scene_region_mode": "auto",
        },
    )
    project.save()
    return project

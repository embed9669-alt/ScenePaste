# ScenePaste V10 · VisionDataForge
...
V10 adds **Asset Studio**, a human-in-the-loop editor for turning segmentation results into reusable data assets. It separates two masks on purpose:

- **Foreground Mask** — the authoritative training label and transparent cutout alpha;
- **Background Removal Mask** — independently editable region used to remove the object, halo and optional shadow when producing a clean background.

Brush add/erase, hole filling, morphology and cleanup helpers are included. Exports keep the automatic mask, edited mask, background-removal mask, original image and provenance metadata. Reviewed foregrounds are written directly into the object library and clean backgrounds into the background library.

Open **Assets → Asset Studio…** (Chinese UI: `素材 → 素材工作室…`). See [`docs/ASSET_STUDIO.md`](docs/ASSET_STUDIO.md).
...
![0 · Cutout Studio](docs/images/gif_00_cutout.gif)
...
- **Optional auto-cutout** — rembg (`pip install -e ".[auto]"`) and **GroundingSAM2** / SAM2 (`pip install -e ".[grounding]"`).
...
Optional automatic foreground extraction (rembg):

```bash
python -m pip install -e ".[auto]"
```

Optional GroundingSAM2 (text-prompt / class-label cutout; needs torch + transformers):

```bash
python -m pip install -e ".[grounding]"
```
...
CLI rembg (no LabelMe JSON required):
...
In the Qt GUI, open **抠图工作室…** (Cutout Studio):

1. **打开文件夹…** — left pane shows the folder path, thumbnail list and count; center shows the selected image  
2. Pick a model (rembg variants / **SAM2 click** / **GroundingSAM2**) → **下载并加载** (visible progress; check hf-mirror in China)  
3. Set the class label → draw / click-to-segment / auto-cutout → **保存当前** (image + LabelMe JSON)

Also available: **自动抠图加载…** (straight into the canvas) and Large Generate’s rembg checkbox.
...
![Cutout Studio](docs/images/ui_cutout_studio.png)

If Hugging Face downloads are slow, set `HF_ENDPOINT=https://hf-mirror.com`. Details: [docs/AUTO_MASK.md](docs/AUTO_MASK.md).
...
- classic point-prompt SAM (outside GroundingSAM2) is not a separate completed backend;

...
class Instance:
    ...
    appearance_hue: float = 0.0          # degrees; 0 = no hue override
    appearance_temperature: float = 0.0  # warm(+)/cool(-); 0 = no override
    appearance_noise: float = 0.0        # gaussian sigma; 0 = no override
    appearance_sharpness: float = 1.0    # 1.0 = no override (<1 soften, >1 sharpen)
    # 图层状态：visible=False 不参与合成；locked=True 画布上拒绝选中/拖拽。
    visible: bool = True
    locked: bool = False
    # 渲染缓存：LRU。不参与相等比较。
    ...
    def appearance_key(self) -> tuple:
        return (
            ...
            round(float(getattr(self, "appearance_hue", 0.0) or 0.0), 3),
            round(float(getattr(self, "appearance_temperature", 0.0) or 0.0), 3),
            round(float(getattr(self, "appearance_noise", 0.0) or 0.0), 3),
            round(float(getattr(self, "appearance_sharpness", 1.0) or 1.0), 3),
        )

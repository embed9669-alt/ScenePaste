...
def record_compose(app: QApplication) -> Path:
    ...
    win.doc._emit()
    _drain(app, 0.25)
    _fit_canvas(win); _bias_splitter(win); _drain(app, 0.2)
...
def record_appearance(app: QApplication) -> Path:
    ...
    win.doc._emit(); _fit_canvas(win); _bias_splitter(win); _drain(app, 0.25)

    frames = []
    steps = [
        {},
        {"appearance_enabled": True, "appearance_recipe": "mild", "appearance_seed": 7},
        {"appearance_brightness": 0.14},
        {"appearance_brightness": -0.1, "appearance_saturation": 1.2},
        {"appearance_contrast": 1.18, "appearance_blur": 0.7},
        {"appearance_seed": 88, "appearance_brightness": 0.0, "appearance_contrast": 1.0,
         "appearance_saturation": 1.0, "appearance_blur": 0.0},
        {"appearance_recipe": "surveillance-object", "appearance_seed": 11},
        {"appearance_recipe": "mild", "appearance_seed": 11, "appearance_saturation": 1.12},
    ]
    for state in steps:
        if state:
...
def record_batch_defaults(app: QApplication) -> Path:
    ...
    win.doc._emit(); _fit_canvas(win); _bias_splitter(win); _drain(app, 0.2)

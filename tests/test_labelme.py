...
def test_write_labelme_json_roundtrip(tmp_path: Path):
    from scenepaste.core.labelme import polygon_shape, write_labelme_json

    img = tmp_path / "obj.jpg"
    _write_image(img)
    json_path = tmp_path / "obj.json"
    poly = [[10, 10], [80, 10], [80, 70], [10, 70]]
    write_labelme_json(
        json_path,
        image_path=img.name,
        image_height=80,
        image_width=100,
        shapes=[polygon_shape("person", poly)],
    )
    data = json.loads(json_path.read_text(encoding="utf-8"))
    assert data["imagePath"] == "obj.jpg"
    assert data["shapes"][0]["label"] == "person"
    assert data["shapes"][0]["shape_type"] == "polygon"
    assert len(data["shapes"][0]["points"]) == 4

    assets = load_object_assets(tmp_path, {"person": 0}, 0.0, lambda _m: None)
    assert len(assets) == 1
    assert assets[0].label == "person"


def test_save_cutout_as_labelme(tmp_path: Path):
    from compose_app_qt.cutout_studio import save_cutout_as_labelme

    src = tmp_path / "src"
    src.mkdir()
    img = src / "car.png"
    bgr = _write_image(img, w=60, h=50)
    out = tmp_path / "objects"
    poly = np.array([[5, 5], [50, 5], [50, 40], [5, 40]], dtype=np.float32)
    json_path = save_cutout_as_labelme(img, out, label="vehicle", polygon=poly, bgr=bgr)
    assert json_path.is_file()
    assert (out / "car.png").is_file()
    assets = load_object_assets(out, {"vehicle": 1}, 0.0, lambda _m: None)
    assert len(assets) == 1
    assert assets[0].label == "vehicle"

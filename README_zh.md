...
![0 · 抠图工作室](docs/images/gif_00_cutout.gif)
...
- **素材搜索 + 响应式缩略图浏览**。
- **LabelMe / X-AnyLabeling 输入**及 polygon 提取；可选 **rembg / GroundingSAM2 自动抠图**。
...
按类别文本抠图（GroundingSAM2，需 torch + transformers）：
...
1. **打开文件夹…** → 左侧显示目录路径、缩略图列表与大图预览  
2. 选模型（多种 rembg / **SAM2 点选** / **GroundingSAM2**）→ **下载并加载**（进度条可见；国内勾选 hf-mirror）  
3. 设类别名 → 手绘多边形 / 点选分割 / 自动抠图 → **保存当前**（图像 + LabelMe JSON）

另有 **自动抠图加载…**（直接进画布）与大规模生成中的 rembg 选项。
...
![抠图工作室](docs/images/ui_cutout_studio.png)

国内 Hugging Face 较慢时可设 `HF_ENDPOINT=https://hf-mirror.com`。详见 [docs/AUTO_MASK.md](docs/AUTO_MASK.md)。
...
- 独立点选式 SAM（不含 GroundingSAM2）仍不是单独完整后端；
...
- [安全策略](SECURITY.md)
- [第三方依赖与许可证说明](THIRD_PARTY_NOTICES.md)

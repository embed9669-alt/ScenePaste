...
class Document:
    ...
    def move_instance(self, uid: int, new_index: int) -> None:
        """Reorder the instances list (which determines canvas z-order).

        Bounds-clamped. Only emits when the position actually changed.
        """
        i = self.index_of_uid(uid)
        if i < 0 or not self.instances:
            return
        new_index = max(0, min(new_index, len(self.instances) - 1))
        if new_index == i:
            return
        instances = list(self.instances)
        inst = instances.pop(i)
        instances.insert(new_index, inst)
        self.instances = instances
        self._emit()

    def bring_to_front(self, uid: int) -> None:
        i = self.index_of_uid(uid)
        if i >= 0:
            self.move_instance(uid, len(self.instances) - 1)

    def send_to_back(self, uid: int) -> None:
        i = self.index_of_uid(uid)
        if i >= 0:
            self.move_instance(uid, 0)

    def set_instance_visibility(self, uid: int, visible: bool) -> None:
        for inst in self.instances:
            if inst.uid == uid:
    ...
    def set_instance_locked(self, uid: int, locked: bool) -> None:
        for inst in self.instances:
            if inst.uid == uid:

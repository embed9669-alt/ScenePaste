...
def load_template(doc, path: Path) -> Tuple[List[Instance], List[str]]:
    ...
    for row in rows:
        ...
        restored.append(Instance(
            ...
            appearance_hue=float(appearance.get("hue", 0.0) or 0.0),
            appearance_temperature=float(appearance.get("temperature", 0.0) or 0.0),
            appearance_noise=float(appearance.get("noise", 0.0) or 0.0),
            appearance_sharpness=float(appearance.get("sharpness", 1.0) or 1.0),
        ))
    if not restored:

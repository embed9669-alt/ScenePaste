...
class LoadCutoutsWorker(QThread):
    ...
    When ``auto_cutout`` is True, uses rembg via
    :func:`scenepaste.core.auto_cutout.load_object_assets_auto` (same path as
    CLI ``--auto-cutout``).
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._auto_cutout = bool(auto_cutout)
        self._auto_cutout_label = auto_cutout_label
        self._auto_cutout_label_from_subdir = bool(auto_cutout_label_from_subdir)
    ...
    def run(self) -> None:
        try:
            ...
            if self._auto_cutout:
                class_map = parse_class_map(self._class_map_text) if self._class_map_text.strip() else {}
                logs: list[str] = []
                ...
                assets = core.load_object_assets_auto(
                    self._objects_dir, class_map, log=_log,
                    label=self._auto_cutout_label,
                    label_from_subdir=self._auto_cutout_label_from_subdir,
                )
                ...
                assets = core.load_object_assets(
                    ...
                    log=lambda _msg: None, rectangle_mask_mode="grabcut",
                )
...
def composite_from_doc(doc, instances=None) -> Image.Image:
    ...
    for inst in insts:
        if not getattr(inst, "visible", True):
            # Hidden layers are excluded from the live composite / save pipeline.
            continue
...
class _InstanceSnap:
    ...
    appearance_hue: float = 0.0
    appearance_temperature: float = 0.0
    appearance_noise: float = 0.0
    appearance_sharpness: float = 1.0
    ...
    def from_instance(cls, inst, bg_w: int, bg_h: int) -> "_InstanceSnap":
        return cls(
            ...
            appearance_hue=float(getattr(inst, "appearance_hue", 0.0) or 0.0),
            appearance_temperature=float(getattr(inst, "appearance_temperature", 0.0) or 0.0),
            appearance_noise=float(getattr(inst, "appearance_noise", 0.0) or 0.0),
            appearance_sharpness=float(getattr(inst, "appearance_sharpness", 1.0) or 1.0),
        )
    ...
    def to_instance(self, bg_w: int, bg_h: int, uid: int = 0):
        ...
        return Instance(
            ...
            appearance_hue=float(getattr(self, "appearance_hue", 0.0) or 0.0),
            appearance_temperature=float(getattr(self, "appearance_temperature", 0.0) or 0.0),
            appearance_noise=float(getattr(self, "appearance_noise", 0.0) or 0.0),
            appearance_sharpness=float(getattr(self, "appearance_sharpness", 1.0) or 1.0),
        )
